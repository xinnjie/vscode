...
export function registerAction2(ctor: { new(): Action2 }): IDisposable {
	...
	// 🔍 LOG: Extract action metadata for documentation
	try {
		...
		// Helper function to convert keybinding number to human-readable string
		const keybindingToString = (kb: number | number[] | undefined, os: OperatingSystem): string | undefined => {
			if (!kb) {
				return undefined;
			}
		...
		if (keybinding) {
			if (Array.isArray(keybinding)) {
				keybindingInfo = keybinding.map(kb => {
					const result: any = { when: kb.when?.serialize ? kb.when.serialize() : kb.when ? String(kb.when) : undefined };
				...
				keybindingInfo = {
					when: keybinding.when?.serialize ? keybinding.when.serialize() : keybinding.when ? String(keybinding.when) : undefined
				};

				if (keybinding.primary || keybinding.win?.primary || keybinding.linux?.primary || keybinding.mac?.primary) {
					keybindingInfo.primary = {
						win: keybindingToString(keybinding.win?.primary || keybinding.primary, OperatingSystem.Windows),
						linux: keybindingToString(keybinding.linux?.primary || keybinding.primary, OperatingSystem.Linux),
						mac: keybindingToString(keybinding.mac?.primary || keybinding.primary, OperatingSystem.Macintosh)
					};
				}

				if (keybinding.secondary || keybinding.win?.secondary || keybinding.linux?.secondary || keybinding.mac?.secondary) {
					const winSecondary = keybinding.win?.secondary || keybinding.secondary;
					const linuxSecondary = keybinding.linux?.secondary || keybinding.secondary;
					const macSecondary = keybinding.mac?.secondary || keybinding.secondary;

					keybindingInfo.secondary = {
						win: winSecondary ? winSecondary.map(k => keybindingToString(k, OperatingSystem.Windows)).filter(Boolean) : undefined,
						linux: linuxSecondary ? linuxSecondary.map(k => keybindingToString(k, OperatingSystem.Linux)).filter(Boolean) : undefined,
						mac: macSecondary ? macSecondary.map(k => keybindingToString(k, OperatingSystem.Macintosh)).filter(Boolean) : undefined
					};
